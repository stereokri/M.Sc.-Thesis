...
parent_dir = Path("/home/kristian/docstore/QA")
...
for chunk in retrieved_chunks:
    doc_id = hashlib.md5(chunk.metadata["title"].encode("utf-8")).hexdigest()
    if doc_id and doc_id not in seen_ids:
        parent_path = parent_dir / doc_id
        if parent_path.exists():
            with open(parent_path, "rb") as f:
                full_text = f.read().decode("utf-8")
                doc_string = (
                    f"<document index='{i}'>\n"
                    f"<source>{chunk.metadata.get('source')}</source>\n"
                    f"<doc_content>{full_text}</doc_content>\n"
                    "</document>\n\n\n\n\n"
                )
